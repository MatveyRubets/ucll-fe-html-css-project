    ...
    btn.addEventListener("click", function () {
      ...
      document.querySelectorAll(".discount-desc").forEach((desc) => {
        if (hasDiscount) {
          if (window.location.pathname.includes("pricing_ua.html")) {
            desc.textContent =
              currentQuantity === 5 ? "5% знижки" : "10% знижки";
          } else {
            desc.textContent =
              currentQuantity === 5 ? "5% zniżki" : "10% zniżki";
          }
      ...
      document.querySelectorAll(".discount-desc-masaj").forEach((desc) => {
        if (hasDiscount && currentQuantity === 5) {
          desc.textContent = window.location.pathname.includes(
            "pricing_ua.html"
          )
            ? "4+1 безкоштовно"
            : "4+1 gratis";
...
document.querySelectorAll(".about__modal-btn").forEach((btn) => {
  btn.addEventListener("click", function () {
    ...
    try {
      ...
      certificates = JSON.parse(btn.dataset.certificates || "[]").map(
        (src) => ({ src, orientation: "vertical" })
      );
    }
